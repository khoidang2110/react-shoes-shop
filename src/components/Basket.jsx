import React from "react";

function Basket(props) {
  const { cartItems, onAdd, onRemove, onDelete } = props;
  const itemsPrice = cartItems.reduce((a, c) => a + c.qty * c.price, 0);
  return (
    <div className="card">
      <div className="cardTop">
        <div className="logoNike">
          <img className="cardLogo" src="../src/assets/nike.png" alt="" />
        </div>
      </div>
      <div className="cardTitle">
        Your cart <span className="cardTitleAmount">$ {itemsPrice.toFixed(2)}  </span>
      </div>
      {cartItems.length === 0 && <div className="cartEmptyText"> Your cart is empty.</div>}
      
        <div className="cardBody">
          <div>
            <div>
            {cartItems.map((item) => (
              <div className="cartItem"  key={item.id} >
                <div className="cartItemLeft">
                  <div className="cartItemImage" style={{ backgroundColor: `${item.color}` }}>
                    <div className="cartItemImageBlock">
                      <img src={item.image} />
                    </div>
                  </div>
                </div>
                <div className="cartItemRight">
                  <div className="cartItemName">
                    {item.name}
                  </div>
                  <div className="cartItemPrice">${item.price}</div>
                  <div className="cartItemActions">
                    <div className="cartItemCount">
                      <div className="cartItemCountButton" onClick={()=> onRemove(item)}>-</div>
                      <div className="cartItemCountNumber">{item.qty}</div>
                      <div className="cartItemCountButton" onClick={()=> onAdd(item)}>+</div>
                    </div>
                    <div className="cartItemRemove" onClick={()=>onDelete(item)}>
                      <img src="data:image/png;base64," />
                    </div>
                  </div>
                </div>
              </div>
                 ))}
            </div>
          </div>
        </div>
   
    </div>
  );
}

export default Basket;
